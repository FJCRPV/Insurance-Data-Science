...
roc_logistic <- roc(logistic_true, logistic_preds_class)
plot(roc_logistic, col = "darkgreen", main = "Logistic Regression ROC Curve")

# Optimize threshold for confusion matrix
best_threshold_logit <- coords(roc_logistic, "best", ret = "threshold", 
                               best.method = "youden")$threshold
logit_preds_class <- ifelse(logistic_preds > best_threshold_logit, 1, 0)
confusionMatrix(as.factor(logistic_preds_class), as.factor(logistic_true), positive="1")  #illustrative purposes only — it's not too informative with imbalanced data
F1_Score(y_pred = logistic_preds_class, y_true = logistic_true, positive = "1")
...
for (k in 1:K) {
  ...
  train_matrix <- model.matrix(large_claim ~ zone+as.factor(power)+agevehicle+agedriver+as.factor(brand)+
                                 fuel -1, 
                               data = train_data)
  test_matrix <- model.matrix(large_claim ~ zone+as.factor(power)+agevehicle+agedriver+as.factor(brand)+
                                fuel -1, 
                              data = test_data)
...
xgb.plot.importance(importance) # Driver and Vehicle Age show great importance, with a big drop for other features
...
xgb_auc_mean2 <- mean(xgb_auc2); xgb_auc_mean2
...
xgb_auc_mean3 <- mean(xgb_auc3); xgb_auc_mean3
